...
export const signin=async(req,res,next)=>{
    ...
    const usertoken=jwt.sign(
        {id:client._id,isAuth:client.isAuth},process.env.JWT_SECERT_KEY
    )
...
export const googleprovider =async(req,res,next)=>{
    ...
    try{
        const client=await User.findOne({email})
        if(client){
            const usetoken=jwt.sign({id:client._id,isAuth:client.isAuth},process.env.JWT_SECERT_KEY)
            const {password,...rest}=client._doc
            res.status(200).cookie('access_token',usetoken,{
                httpOnly:true
            }).json(rest)
        }else{
        ...
        const usetoken=jwt.sign({id:newclient._id,isAuth:newclient.isAuth},process.env.JWT_SECERT_KEY)
...
export const registermailsubmit = async (req, res, next) => {
    try {
        ...
        if (!post) return next(errormsg(404, "Post not found"));
        ...
        const emailSent = await sendRegistrationEmail(email, name, post.title || "Unknown Post");
        if (!emailSent) {
